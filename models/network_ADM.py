...
class FourLayer_64F(nn.Module):
	def __init__(self, norm_layer=nn.BatchNorm2d, num_classes=5, neighbor_k=3, batch_size=4, shot_num=1):
		...
		self.classifier = ADM_Metric(neighbor_k=neighbor_k, batch_size=batch_size, shot_num=shot_num)  # 1*num_classes
...
class ADM_Metric(nn.Module):
	...
	def cal_ADM_similarity(self, input1_batch, input2_batch):
		'''
		...
		for i in range(self.batch_size):
			...
			input1 = input1_batch[i]     # 75 * 441 * 64
			input2 = input2_batch[i]     # 25 * 441 * 64
			...
			kl_sim_soft2 = torch.cat((kl_dis, inner_sim), 1)#.unsqueeze(1)
	...
	def forward(self, x1, x2):
		...
		Similarity_list, Q_S_List = self.cal_ADM_similarity(x1, x2)
