...
class FourLayer_64F(nn.Module):
	def __init__(self, norm_layer=nn.BatchNorm2d, num_classes=5, neighbor_k=3, batch_size=4, shot_num=1):
		...
		self.classifier = KL_Metric(neighbor_k=neighbor_k, batch_size=batch_size, shot_num=shot_num)  # 1*num_classes
...
class KL_Metric(nn.Module):
	def __init__(self, neighbor_k=3, batch_size=4, shot_num=1):
		super(KL_Metric, self).__init__()
	...
	def cal_KL_Similarity(self, input1_batch, input2_batch):
		'''
	...
	def forward(self, x1, x2):
		...
		Similarity_list, Q_S_List = self.cal_KL_Similarity(x1, x2)
