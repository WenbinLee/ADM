...
class FourLayer_64F(nn.Module):
	def __init__(self, norm_layer=nn.BatchNorm2d, num_classes=5, neighbor_k=3, batch_size=4, shot_num=1):
		...
		self.classifier = KL_CMS_Metric(neighbor_k=neighbor_k, batch_size=batch_size, shot_num=shot_num)  # 1*num_classes
...
class KL_CMS_Metric(nn.Module):
	def __init__(self, neighbor_k=3, batch_size=4, shot_num=1):
		super(KL_CMS_Metric, self).__init__()
	...
	def cal_KL_CMS_Similarity(self, input1_batch, input2_batch):
		'''
		...
		for i in range(self.batch_size):
			...
			kl_dis1 = -self.KL_distance_Batch(query_mean, query_cov, s_mean, s_cov)  # 75 * 5
			kl_dis2 = self.KL_distance_Batch(query_mean, query_cov, s_remain_mean, s_remain_cov)  # 75 * 5
			Similarity_list.append(kl_dis1+kl_dis2)
	...
	def forward(self, x1, x2):
		...
		Similarity_list, Q_S_List = self.cal_KL_CMS_Similarity(x1, x2)
